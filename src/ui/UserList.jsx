...
export default function UserList({ users, onUserIdClick, onPersonClick, onTokenClick, onDeleteUser }) {
  const [showModal, setShowModal] = useState(false);
  const [toDeleteId, setToDeleteId] = useState(null);

  function handleAskDelete(userId) {
          ...
          {users.map(user => (
            <tr key={user.id}>
              <td>
                <Button
                  variant="link"
                  onClick={() => window.open(`/faire-part/admin-panel/user/${user.id}`, '_blank', 'noopener')}
                ...
                {user.personnes && user.personnes.length > 0 ? (
                  user.personnes.map((p, idx) => (
                    <Badge
                      ...
                      onClick={e => {
                        e.stopPropagation();
                        window.open(`/faire-part/admin-panel/user/${user.id}`, '_blank', 'noopener');
                      }}
                    >
                      {p.prenom} {p.nom}
                    </Badge>
              ...
              <td>
                <Button
                  ...
                  size="sm"
                  onClick={e => { e.stopPropagation(); onTokenClick(user.login_token); }}
