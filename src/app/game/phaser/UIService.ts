import Phaser from 'phaser';

interface NPC { 
  action?: () => void; 
  objectiveId?: number 
}

interface ChecklistService { 
  markDone: (objectiveId: number) => void 
}

interface InputService { 
  events: Phaser.Events.EventEmitter 
}

interface ActionButtonExtended extends Phaser.GameObjects.Image {
  active: boolean;
  npc: NPC | null;
}

export default class UIService {
  private scene: Phaser.Scene;
  private checklistService: ChecklistService | null;
  private inputService: InputService;
  private actionButton: ActionButtonExtended | null = null;

  constructor(scene: Phaser.Scene, checklistService: ChecklistService | null, inputService: InputService) {
    this.scene = scene;
    this.checklistService = checklistService;
    this.inputService = inputService;
    this.createActionButton();
    this.inputService.events.on('action', () => this.handleAction());
  }

  private createActionButton() {
    const margin = 10;
    const uiScale = 2;
    if (!this.scene.textures.exists('ui-interface')) return;
    
    const button = this.scene.add.image(
      this.scene.scale.width - margin - 48, 
      this.scene.scale.height - margin - 48 * 1.5, 
      'ui-interface', 
      'button_action'
    )
      .setInteractive()
      .setScrollFactor(0)
      .setDepth(150)
      .setAlpha(0.4)
      .setScale(uiScale)
      .on('pointerdown', () => { this.inputService.events.emit('action'); }) as ActionButtonExtended;
    
    button.active = false;
    button.npc = null;
    this.actionButton = button;
  }

  showActionButton(active: boolean, npc: NPC | null = null) {
    if (!this.actionButton) return;
    this.actionButton.npc = npc;
    if (active) { 
      this.actionButton.setAlpha(1); 
      this.actionButton.active = true; 
    } else { 
      this.actionButton.setAlpha(0.4); 
      this.actionButton.active = false; 
      this.actionButton.npc = null; 
    }
  }

  private handleAction() {
    if (!this.actionButton || !this.actionButton.active) return;
    const npc: NPC | null = this.actionButton.npc;
    if (!npc) return;
    if (npc.action) npc.action();
    if (npc.objectiveId && this.checklistService) this.checklistService.markDone(npc.objectiveId);
    this.showActionButton(false);
  }
}
