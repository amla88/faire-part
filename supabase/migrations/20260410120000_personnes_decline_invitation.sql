-- Refus d'invitation par personne (RSVP) : en plus des cases par moment (réception / repas / soirée).

ALTER TABLE public.personnes
  ADD COLUMN IF NOT EXISTS decline_invitation boolean NOT NULL DEFAULT false;

COMMENT ON COLUMN public.personnes.decline_invitation IS
  'Si vrai, l''invité a décliné l''invitation : aucune présence aux moments, champs banquet vidés.';
